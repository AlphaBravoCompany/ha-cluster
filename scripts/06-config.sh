...
4. Paste the following into the /etc/rancher/rke2/config.yaml file
-----
...
8. Repeat steps 1-7 for the remaining CP-3 server node, changing cp-2 to cp-3.
-----
...
4. Paste the following into the /etc/rancher/rke2/config.yaml file. Note the difference in the config.
-----
server: https://${node_ips[lb-cp]}:9345
token: QMXrt0w8BWFO5Z1D0zLm5VKq5HF74Yl8EE9IR72YUqUtaeW4Xix1tCgdzJU0meE5
...
6. Reload the Systemctl daemon and enable the rke2-agent. It will take a few moments to come online.
-----
sudo systemctl daemon-reload
sudo systemctl enable --now rke2-agent


7. Verify the worker node has come online
-----
exit
ssh cp-1
sudo /var/lib/rancher/rke2/bin/kubectl --kubeconfig /etc/rancher/rke2/rke2.yaml get nodes
